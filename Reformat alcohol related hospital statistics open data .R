...
arhs.format <- function(x,y) {
  ...
  pipe$Measurement <- ifelse(colnames(x)[y]=="EASRStays", "Ratio",                 # nested ifelse statements to assign the correct measurement type to the various types of arhs data
                             ...
                             "Count")))))))
  pipe$Units <- ifelse(colnames(x)[y]=="EASRStays", "Stays Per 100,000 Population",        # nested ifelse statements to assign the correct measurement units to the various types of arhs data
                       (ifelse(colnames(x)[y]=="EASRPatients", "Patients Per 100,000 Population",
                       (ifelse(colnames(x)[y]=="EASRNewPatients", "New Patients Per 100,000 Population",
